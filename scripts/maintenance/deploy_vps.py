...
def deploy():
    ...
    try:
        ...
        commands = [
            ...
            {
                'desc': 'Reconstruyendo frontend y backend',
                'cmd': 'cd /root/fastisp && docker compose -f docker-compose.prod.yml --env-file .env.prod build backend celery-worker celery-beat frontend'
            },
            {
                'desc': 'Reiniciando contenedores',
                'cmd': 'cd /root/fastisp && docker compose -f docker-compose.prod.yml --env-file .env.prod up -d --no-deps backend celery-worker celery-beat frontend'
            }
        ]
