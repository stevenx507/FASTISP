...
const MikroTikManagement: React.FC = () => {
  ...
  const runConnectionExpress = async () => {
    ...
    const registerWireGuardPeerStep = async (stepId: string, configureRouterPeer = false) => {
      updateStep(stepId, 'running')
      try {
        const response = await apiFetch(`/api/mikrotik/routers/${selectedRouter.id}/wireguard/register-peer`, {
          ...
          body: JSON.stringify(
            withChangeTicket({
              ...
              configure_router_peer: configureRouterPeer,
            })
          ),
        })
        ...
        if (response.ok && payload?.success) {
          ...
          updateStep(stepId, 'success', configureRouterPeer ? `Router+VPS sincronizados (${mode})` : `Peer registrado en VPS (${mode})`)
          return true
        }
    ...
    const baseSteps: ExpressStepState[] = profile === 'public'
      ? [
          ...
          { id: 'wg_tunnel', label: 'Autoenlace WireGuard', status: 'pending' },
          { id: 'wg_register_peer', label: 'Sincronizar peer en VPS', status: 'pending' },
    ...
    try {
      ...
      if (!connected) {
        const autoLinked = await registerWireGuardPeerStep('wg_tunnel', true)
        if (autoLinked) {
          updateStep('wg_register_peer', 'skipped', 'Incluido en autoenlace')
          connected = await testConnectionStep('verify_wg', 'Conexion por WireGuard operativa')
        } else {
          const wgOk = await executeScriptStep('wg_tunnel', 'tunel WireGuard (fallback script)', quickConnect.scripts.wireguard_site_to_vps_script || '')
          if (wgOk) {
            const registered = await registerWireGuardPeerStep('wg_register_peer', false)
            if (registered) {
              connected = await testConnectionStep('verify_wg', 'Conexion por WireGuard operativa')
            } else {
