...
const PlanChangeModal: React.FC<Props> = ({ clientId, open, onClose, onChanged }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur">
      <div className="bg-white/95 border border-gray-200 rounded-2xl shadow-2xl w-full max-w-lg p-6 text-slate-800">
          ...
          {plans.map((p) => (
            <label
              ...
              className={`flex items-center justify-between rounded-xl border px-4 py-3 cursor-pointer ${
                selectedPlan === p.id ? 'border-cyan-400 bg-white' : 'border-gray-200 hover:border-white/20'
              }`}
            >
              <div>
                <p className="font-semibold">{p.name}</p>
                <p className="text-sm text-slate-600">${p.price?.toFixed(2) ?? '0.00'}</p>
              </div>
              <input type="radio" name="plan" value={p.id} checked={selectedPlan === p.id} onChange={() => setSelectedPlan(p.id)} />
            </label>
          ))}
          {plans.length === 0 && <p className="text-sm text-slate-600">No hay planes configurados.</p>}
        </div>
        <div className="mt-4 flex items-center gap-2">
          <input id="prorate" type="checkbox" className="h-4 w-4" checked={prorate} onChange={(e) => setProrate(e.target.checked)} />
          <label htmlFor="prorate" className="text-sm text-slate-700">
