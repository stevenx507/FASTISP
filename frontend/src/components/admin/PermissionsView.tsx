    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">RBAC por rol para controlar acceso por modulo y accion.</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <form onSubmit={saveOverride} className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-4 shadow-sm">
          ...
          <div className="divide-y divide-white/5">
            {roles.map((item) => (
              <div key={item.role} className="px-4 py-3">
                ...
                {!item.wildcard && (
                  <p className="mt-2 text-xs text-slate-400">{item.permissions.slice(0, 10).join(', ') || 'sin permisos'}</p>
                )}
              </div>
            ))}
            {!roles.length && <div className="px-4 py-8 text-sm text-slate-400">Sin datos.</div>}
          </div>
        </div>
          ...
          <div className="max-h-96 overflow-auto divide-y divide-white/5">
            {overrides.map((item) => (
              <div key={item.id} className="px-4 py-3 text-sm">
                <p className="font-semibold text-white">{item.role}</p>
                <p className="text-xs text-slate-400">{item.permission}</p>
            ...
            {!overrides.length && <div className="px-4 py-8 text-sm text-slate-400">Sin overrides.</div>}
          </div>
        </div>
      </div>
    </div>
