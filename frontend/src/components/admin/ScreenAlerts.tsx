...
const statusColor: Record<AlertStatus, string> = {
  ...
  expired: 'bg-white/10 text-slate-600',
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white">Avisos en pantalla</h2>
          <p className="text-sm text-slate-500">Banners operativos para portal cliente por audiencia y severidad.</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-red-100 bg-rose-500/10 p-4">
          <p className="text-xs font-semibold uppercase text-rose-400">Critical</p>
          <p className="mt-2 text-2xl font-bold text-rose-300">{items.filter((item) => item.severity === 'critical').length}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <form onSubmit={createAlert} className="rounded-xl border border-gray-200 bg-white backdrop-blur-md p-4 shadow-sm">
          <h3 className="mb-3 font-semibold text-white">Nuevo aviso</h3>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <select
                ...
                className="rounded-lg border border-white/20 px-2 py-2 text-xs"
              >
        ...
        <div className="rounded-xl border border-gray-200 bg-white backdrop-blur-md shadow-sm xl:col-span-2">
          <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
            <h3 className="font-semibold text-white">Historial y estado</h3>
            <select
              value={filter}
          ...
          <div className="divide-y divide-white/5">
            ...
            {!filteredItems.length && (
              <div className="px-4 py-8 text-center text-sm text-slate-500">Sin avisos para este filtro.</div>
            )}
          </div>
        </div>
      </div>
