  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <Handshake className="w-12 h-12 text-blue-500 animate-pulse" />
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">Portal de Aliados</h1>
              <p className="text-slate-400">Bienvenido, {partnerData?.name}</p>
            </div>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <button 
              onClick={() => setShowModal(true)}
              className="flex-1 md:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl transition-all font-semibold flex items-center justify-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl backdrop-blur-sm">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xs font-bold text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-full">DISPONIBLE</span>
            </div>
            <p className="text-slate-400 text-sm mb-1">Comisiones por Cobrar</p>
            <p className="text-3xl font-bold">${partnerData?.balance?.toFixed(2) || '0.00'}</p>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl backdrop-blur-sm">
            ...
            <p className="text-3xl font-bold">{partnerData?.totalSales || 0}</p>
          </div>

          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl backdrop-blur-sm">
            <div className="flex justify-between items-center mb-4">
              <Award className="w-6 h-6 text-indigo-400" />
              <span className="text-xs font-bold text-indigo-400 bg-indigo-400/10 px-2 py-1 rounded-full">{partnerData?.rank || 'Bronze'}</span>
            </div>
            <p className="text-slate-400 text-sm mb-1">Nivel de Aliado</p>
            <p className="text-3xl font-bold">{partnerData?.commission_rate || 0}%</p>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-800">
            {commissions.length > 0 ? commissions.map((comm: any) => (
              <div key={comm.id} className="p-4 flex justify-between items-center hover:bg-slate-800/30 transition-colors">
                <div className="flex items-center gap-4">
                  <div className={`w-2 h-2 rounded-full ${comm.status === 'paid' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                  <div>
                    <p className="font-medium">{comm.client_name || 'Desconocido'}</p>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md overflow-hidden">
              ...
              <form onSubmit={handleRegisterProspect} className="p-6 space-y-4">
                <div>
                  ...
                  <input 
                    type="text" required
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 focus:border-blue-500 outline-none"
                ...
                <div>
                  ...
                  <textarea 
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 focus:border-blue-500 outline-none h-24"
                    value={newProspect.address}
                    onChange={e => setNewProspect({...newProspect, address: e.target.value})}
                  />
                </div>
                <button type="submit" className="w-full py-4 bg-blue-600 rounded-2xl font-bold hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20">
                  Registrar Prospecto
                </button>
              </form>
            </div>
