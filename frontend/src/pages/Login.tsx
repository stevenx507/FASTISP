...
const Login: React.FC = () => {
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Error al autenticar con Google'
      toast.error(msg)
    } finally {
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          <form onSubmit={handleLogin} className="space-y-8">
            ...
            <button
              ...
              className="w-full py-5 px-8 rounded-2xl bg-white border-2 border-slate-100 text-slate-900 font-black text-sm uppercase tracking-[0.3em] hover:bg-slate-50 hover:border-slate-200 transition-all flex items-center justify-center gap-4 shadow-sm"
            >
              {isGoogleLoading ? (
                <div className="h-4 w-4 border-2 border-[#FF6961]/30 border-t-[#FF6961] rounded-full animate-spin" />
              ) : (
