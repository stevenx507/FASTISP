...
def _validate_password_policy(password: str, tenant_id) -> tuple[bool, str]:
    ...
    has_letter = any(ch.isalpha() for ch in secret)
    has_digit = any(ch.isdigit() for ch in secret)
    if not (has_letter and has_digit):
        return False, "La contrasena debe incluir letras y numeros."
    return True, ""


def _generate_user_password(tenant_id=None, length: int | None = None) -> str:
    minimum = _password_policy_min_length(tenant_id)
    target_length = max(minimum, int(length or 14), 12)
...
def platform_create_tenant():
    ...
    if admin_email:
        ...
        admin_password = str(data.get('admin_password') or '').strip() or _generate_user_password(tenant.id)
        valid_password, password_error = _validate_password_policy(admin_password, tenant.id)
        if not valid_password:
            db.session.rollback()
            return jsonify({"error": password_error}), 400
        admin_user = User(
...
def platform_create_tenant_admin(tenant_id):
    ...
    password = str(data.get('password') or '').strip() or _generate_user_password(tenant.id)
...
def admin_create_client():
    ...
    if create_portal_access:
        ...
        if User.query.filter_by(email=email).first():
            return jsonify({"error": "email ya existe"}), 409
        generated_password = requested_password or _generate_user_password(tenant_id)
        user = User(
            name=name,
            email=email,
            role='client',
...
def _create_client_from_payload(payload: dict, tenant_id) -> tuple[Client, User | None, str | None]:
    ...
    if payload["create_portal_access"]:
        generated_password = payload["requested_password"] or _generate_user_password(tenant_id)
        user = User(
            name=payload["name"],
            email=payload["email"],
            role='client',
...
def _apply_bulk_update_payload(payload: dict, tenant_id) -> dict:
    ...
    if portal_mode == 'create':
        generated_password = payload.get("portal_password") or _generate_user_password(
            tenant_id if tenant_id is not None else client.tenant_id
        )
        user = User(
...
def admin_manage_client_portal_access(client_id):
    ...
    email = str(data.get('email') or '').strip().lower()
    requested_password = str(data.get('password') or '').strip()
    portal_tenant_id = tenant_id if tenant_id is not None else client.tenant_id
    generated_password = requested_password or _generate_user_password(portal_tenant_id)
...
def admin_staff_create():
    ...
    temporary_password = supplied_password or _generate_user_password(tenant_id)
