...
def _parse_wireguard_text_payload(raw_text: str) -> Dict[str, Any]:
    text = str(raw_text or '').strip()
    if not text:
        return _parse_wireguard_config('')
    ...
    parsed = _parse_wireguard_config(normalized)
    if parsed.get('is_wireguard_config'):
        return parsed

    private_key_match = re.search(r'(?im)(?:privatekey|private_key)\s*[:=]\s*([A-Za-z0-9+/=]{20,})', normalized)
    public_key_match = re.search(r'(?im)(?:publickey|public_key)\s*[:=]\s*([A-Za-z0-9+/=]{20,})', normalized)
    endpoint_match = re.search(r'(?im)(?:endpoint)\s*[:=]\s*([^\s,;]+)', normalized)
    allowed_ips_match = re.search(r'(?im)(?:allowedips|allowed_ips)\s*[:=]\s*([^\n]+)', normalized)
    dns_match = re.search(r'(?im)(?:dns)\s*[:=]\s*([^\n]+)', normalized)
    address_match = re.search(r'(?im)(?:address)\s*[:=]\s*([^\n]+)', normalized)
    keepalive_match = re.search(r'(?im)(?:persistentkeepalive|persistent_keepalive)\s*[:=]\s*([^\n]+)', normalized)

    private_key = str(private_key_match.group(1) if private_key_match else '').strip()
    public_key = str(public_key_match.group(1) if public_key_match else '').strip()
    if not private_key or not public_key:
        return parsed

    endpoint = str(endpoint_match.group(1) if endpoint_match else '').strip()
    endpoint_payload = _parse_wireguard_endpoint(endpoint)
    allowed_ips = _split_csv_values(str(allowed_ips_match.group(1) if allowed_ips_match else ''))
    dns_values = _split_csv_values(str(dns_match.group(1) if dns_match else ''))
    addresses = _split_csv_values(str(address_match.group(1) if address_match else ''))
    keepalive = str(keepalive_match.group(1) if keepalive_match else '').strip()

    return {
        'is_wireguard_config': True,
        'interface_private_key': private_key,
        'interface_addresses': addresses,
        'interface_dns': dns_values,
        'peer_public_key': public_key,
        'peer_allowed_ips': allowed_ips,
        'peer_persistent_keepalive': keepalive,
        'endpoint': endpoint_payload.get('endpoint', ''),
        'endpoint_host': endpoint_payload.get('host', ''),
        'endpoint_port': endpoint_payload.get('port'),
    }
...
def _is_global_ipv4(host: str) -> bool:
    try:
        ip_obj = ipaddress.ip_address(str(host or '').strip())
        return ip_obj.version == 4 and ip_obj.is_global
    except ValueError:
        return False


def _discover_vps_wireguard_public_key(runtime: Dict[str, Any]) -> str:
    interface_name = str(runtime.get('vps_interface') or WG_VPS_INTERFACE_DEFAULT).strip() or WG_VPS_INTERFACE_DEFAULT
    argv = ['wg', 'show', interface_name, 'public-key']
    mode = str(runtime.get('mode') or WG_VPS_SYNC_MODE_DEFAULT)
    timeout_seconds = int(runtime.get('ssh_timeout_seconds') or 8)

    if mode in ('auto', 'local'):
        local_result = _run_local_command(argv, timeout_seconds=timeout_seconds)
        if local_result.get('ok'):
            raw_output = str(local_result.get('stdout') or '').strip()
            candidate = raw_output.splitlines()[0] if raw_output else ''
            if _is_valid_wireguard_public_key(candidate):
                return candidate
        if mode == 'local':
            return ''
    ...
    if mode in ('auto', 'ssh'):
        ...
        if bool(runtime.get('ssh_use_sudo')):
        ...
        if runtime.get('ssh_key_path'):
            connect_kwargs['key_filename'] = runtime.get('ssh_key_path')
        if runtime.get('ssh_password'):
            connect_kwargs['password'] = runtime.get('ssh_password')
        try:
            client.connect(**connect_kwargs)
            result = _run_ssh_command(client, command, timeout_seconds=timeout_seconds)
            if not result.get('ok') and bool(runtime.get('ssh_use_sudo')):
                result = _run_ssh_command(client, ' '.join(shlex.quote(item) for item in argv), timeout_seconds=timeout_seconds)
            if result.get('ok'):
...
def _ensure_router_wireguard_site_peer(
    service: MikroTikService,
    ...
    listen_port_raw = data.get('router_listen_port') if data.get('router_listen_port') not in (None, '') else 13231
    try:
        listen_port = int(listen_port_raw)
    except (TypeError, ValueError):
        listen_port = 13231
    listen_port = min(max(listen_port, 1), 65535)

    preferred_router_ip = _normalize_wg_allowed_ip(data.get('router_allowed_ip') or data.get('router_address') or data.get('allowed_ip'))
    if not preferred_router_ip:
        preferred_router_ip = _safe_router_wireguard_allowed_ip(router.id)

    profile = _resolve_wireguard_profile(data)
    runtime = _resolve_wg_sync_runtime(data)
    auto_discovered_server_key = ''
    if not bool(profile.get('server_public_key_valid')):
        auto_discovered_server_key = _discover_vps_wireguard_public_key(runtime)
        if _is_valid_wireguard_public_key(auto_discovered_server_key):
            profile['server_public_key'] = auto_discovered_server_key
            profile['server_public_key_valid'] = True
            profile['issues'] = [str(item) for item in (profile.get('issues') or []) if 'public key valida' not in str(item)]

    endpoint_host = str(profile.get('endpoint_host') or '').strip()
    endpoint_port = int(profile.get('endpoint_port') or 51820)
    if not endpoint_host:
        return {'success': False, 'error': 'No hay endpoint WireGuard configurado para el VPS', 'profile': profile}

    ssh_host = str(runtime.get('ssh_host') or '').strip()
    endpoint_source = str((profile.get('source') or {}).get('endpoint') or '')
    prefer_ssh_host_endpoint = _as_bool(data.get('prefer_ssh_host_endpoint'), default=True)
    if (
        prefer_ssh_host_endpoint
        and _is_global_ipv4(ssh_host)
        and endpoint_source in ('default', 'env')
        and not _is_global_ipv4(endpoint_host)
    ):
        endpoint_host = ssh_host

    server_public_key = str(profile.get('server_public_key') or '').strip()
    if not _is_valid_wireguard_public_key(server_public_key):
        return {
            'success': False,
            'error': 'No hay public key WireGuard valida del VPS (configura perfil WG o habilita auto-sync).',
            'profile': profile,
        }

    allowed_subnets = str(profile.get('allowed_subnets') or WG_PROFILE_ALLOWED_SUBNETS_DEFAULT).strip() or WG_PROFILE_ALLOWED_SUBNETS_DEFAULT

    try:
        interfaces_api = api_obj.get_resource('/interface/wireguard')
        address_api = api_obj.get_resource('/ip/address')
        peers_api = api_obj.get_resource('/interface/wireguard/peers')
        firewall_api = api_obj.get_resource('/ip/firewall/filter')
    except Exception as exc:
        return {'success': False, 'error': f'No se pudieron cargar recursos API WireGuard: {exc}'}

    try:
        existing_interface_rows = interfaces_api.get(name=router_interface) or []
        if not existing_interface_rows:
            interfaces_api.add(name=router_interface, listen_port=str(listen_port), comment='FastISP NOC tunnel')
            existing_interface_rows = interfaces_api.get(name=router_interface) or []
        if not existing_interface_rows:
            return {'success': False, 'error': f'No se pudo crear/leer interfaz {router_interface}'}
        interface_row = existing_interface_rows[0]
        interface_id = str(_pick_value(interface_row, '.id', 'id') or '').strip()
        current_listen = str(_pick_value(interface_row, 'listen-port', 'listen_port') or '').strip()
        if interface_id and current_listen and current_listen != str(listen_port):
            interfaces_api.set(id=interface_id, listen_port=str(listen_port))
    except Exception as exc:
        return {'success': False, 'error': f'No se pudo preparar interfaz WireGuard del router: {exc}'}

    try:
        addresses = address_api.get(interface=router_interface) or []
        normalized_existing = [_normalize_wg_allowed_ip(_pick_value(item, 'address')) for item in addresses]
        if preferred_router_ip not in normalized_existing:
            address_api.add(address=preferred_router_ip, interface=router_interface, comment='FastISP tunnel')
    except Exception as exc:
        return {'success': False, 'error': f'No se pudo configurar IP del tunel en router: {exc}'}

    try:
        peer_rows = peers_api.get(interface=router_interface) or []
        target_peer = None
        duplicate_ids: List[str] = []
        for row in peer_rows:
            peer_id = str(_pick_value(row, '.id', 'id') or '').strip()
            peer_comment = str(_pick_value(row, 'comment') or '').strip()
            peer_key = str(_pick_value(row, 'public-key', 'public_key') or '').strip()
            if peer_comment == 'FastISP NOC peer' or peer_key == server_public_key:
                if target_peer is None:
                    target_peer = row
                elif peer_id:
                    duplicate_ids.append(peer_id)

        if target_peer is not None:
            peer_id = str(_pick_value(target_peer, '.id', 'id') or '').strip()
            if peer_id:
                peers_api.set(
                    id=peer_id,
                    interface=router_interface,
                    public_key=server_public_key,
                    endpoint_address=endpoint_host,
                    endpoint_port=str(endpoint_port),
                    allowed_address=allowed_subnets,
                    persistent_keepalive='25s',
                    comment='FastISP NOC peer',
                )
        else:
            peers_api.add(
                interface=router_interface,
        ...
        for peer_id in duplicate_ids:
            try:
                peers_api.remove(id=peer_id)
            except Exception:
                pass
    except Exception as exc:
        return {'success': False, 'error': f'No se pudo configurar peer WireGuard en router: {exc}'}

    try:
        firewall_rows = firewall_api.get(
            chain='input',
            action='accept',
            protocol='udp',
    ...
    return {
        ...
        'router_allowed_ip': preferred_router_ip,
        'endpoint_host': endpoint_host,
        'endpoint_port': endpoint_port,
        'server_public_key': server_public_key,
        'server_public_key_auto_discovered': bool(auto_discovered_server_key),
        'profile': profile,
    }
...
def _resolve_bth_key_for_request(
    ...
    managed_identity = _get_or_create_managed_bth_identity(
...
def router_quick_connect(router_id):
    ...
    safe_wg_server_public_key = _script_escape(wg_server_public_key)
    safe_wg_endpoint_host = _script_escape(wg_endpoint_host)
    safe_wg_allowed_subnets = _script_escape(wg_allowed_subnets)
    ...
    scripts = {
        ...
        'wireguard_site_to_vps_script': (
            ...
            + f'/interface/wireguard/peers/add interface=$wgName public-key="{safe_wg_server_public_key}" endpoint-address={safe_wg_endpoint_host} endpoint-port={wg_endpoint_port} allowed-address="{safe_wg_allowed_subnets}" persistent-keepalive=25s comment="FastISP NOC peer"\n'
...
def register_router_wireguard_peer(router_id):
    ...
    data = request.get_json(silent=True) or {}
    configure_router_peer = _as_bool(data.get('configure_router_peer'), default=False)
    router_interface = str(data.get('router_interface') or data.get('router_wg_interface') or 'wg-fastisp').strip() or 'wg-fastisp'
    ...
    try:
        with MikroTikService(router.id) as service:
            ...
            if configure_router_peer:
                router_config = _ensure_router_wireguard_site_peer(service, router, data)
                if not router_config.get('success'):
                    return jsonify(
                        {
                            'success': False,
                            'error': router_config.get('error') or 'Could not configure router WireGuard peer',
    ...
    router_peer_ip = str(router_config.get('router_allowed_ip') or router_identity.get('selected_peer_ip') or '').strip()
    ...
    payload = {
        ...
        'router_config': router_config if configure_router_peer else None,
        'vps_sync': {
            'success': bool(sync_result.get('success')),
...
def add_back_to_home_user(router_id):
    ...
    raw_allow_lan = data.get('allow_lan')
    if raw_allow_lan in (None, ''):
        return jsonify({'success': False, 'error': 'allow_lan must be provided explicitly'}), 400
    allow_lan_token = str(raw_allow_lan).strip().lower()
    if allow_lan_token not in ('1', 'true', 'yes', 'y', 'on', '0', 'false', 'no', 'n', 'off'):
        return jsonify({'success': False, 'error': 'allow_lan must be boolean'}), 400
    allow_lan = allow_lan_token in ('1', 'true', 'yes', 'y', 'on')

    user_name = str(data.get('user_name') or data.get('name') or '').strip() or 'noc-vps'
    key_resolution = _resolve_bth_key_for_request(
        str(data.get('private_key') or ''),
        preferred_user_name=user_name,
