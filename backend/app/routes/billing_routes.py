...
def billing_summary():
    ...
    for s in subs.all():
        ...
        invoices.append({
            ...
            "currency": s.currency,
            "due": s.next_charge.isoformat() if s.next_charge else datetime.now(timezone.utc).date().isoformat(),
...
@billing_bp.route('/subscriptions/run-reminders', methods=['POST'])
@admin_required()
...
def payments_webhook():
    ...
    if status in {'paid', 'succeeded'} and sub_id:
        ...
        if sub:
            sub.status = 'active'
            sub.next_charge = (sub.next_charge or datetime.now(timezone.utc).date()) + timedelta(days=sub.cycle_months * 30)
            db.session.add(sub)
